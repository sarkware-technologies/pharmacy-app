...
const DoctorRegistrationForm = ({ onSaveDraftRef }) => {
    ...
    try {
      ...
      const buildDraftPayload = () => {
        ...
        if (formData.doctorName && formData.doctorName.trim()) {
          generalDetails.name = formData.doctorName.trim();
        }
        if (formData.clinicName && formData.clinicName.trim()) {
          generalDetails.clinicName = formData.clinicName.trim();
        }
        if (formData.speciality && formData.speciality.trim()) {
          generalDetails.specialist = formData.speciality.trim();
        }
        ...
        // Add mapping if there are selected hospitals or pharmacies
        ...
        } else if (formData.selectedPharmacies && formData.selectedPharmacies.length > 0) {
          draftPayload.mapping = {
      ...
      const hasFormData =
        ...
        (formData.clinicRegistrationNumber && formData.clinicRegistrationNumber.trim()) ||
        (formData.practiceLicenseNumber && formData.practiceLicenseNumber.trim()) ||
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      {/* Custom Header - Only for Edit Mode */}
      {inEditMode && (
