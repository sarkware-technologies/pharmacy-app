...
const GovtHospitalRegistrationForm = ({ onSaveDraftRef }) => {
    ...
    try {
      ...
      const buildDraftPayload = () => {
        ...
        if (formData.hospitalName && formData.hospitalName.trim()) {
          generalDetails.name = formData.hospitalName.trim();
        }
        ...
        if (formData.registrationNumber && formData.registrationNumber.trim()) {
          licenceDetails.licence.push({
            ...
            hospitalCode: formData.registrationNumber.trim() || '',
          });
        }
        ...
        // Add mapping if there are linked hospitals or pharmacies
        ...
        } else if (formData.linkedPharmacies && formData.linkedPharmacies.length > 0) {
          draftPayload.mapping = {
        ...
        if (stockists && stockists.length > 0 && stockists[0].name) {
          draftPayload.suggestedDistributors = stockists
      ...
      const hasFormData =
        ...
        (formData.linkedHospitals && formData.linkedHospitals.length > 0) ||
  ...
  const populateFormFromCustomerData = (data) => {
    try {
      ...
      if (registrationDoc) {
        setDocumentIds(prev => [...prev, { field: 'registrationCertificateFile', docId: registrationDoc.docId }]);
      }
      if (hospitalImageDoc) {
        setDocumentIds(prev => [...prev, { field: 'hospitalImageFile', docId: hospitalImageDoc.docId }]);
      }
      if (panDoc) {
  ...
  useEffect(() => {
    ...
    else if (!formData.nin || formData.nin.trim().length === 0) isValid = false;
    ...
    else if (!formData.address2 || formData.address2.trim().length === 0) isValid = false;
    else if (!formData.address3 || formData.address3.trim().length === 0) isValid = false;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        console.log(isEditMode ? 'Update successful:' : 'Registration successful:', response?.data?.id);

        Toast.show({
          type: 'success',
          text1: isEditMode ? 'Update Successful' : 'Registration Successful',
          text2: response.message || (isEditMode ? 'Customer details updated successfully' : 'Government Hospital registered successfully'),
          position: 'top',
        });
          ...
          {formData.linkedHospitals.map((hospital, index) => (
            <View key={hospital.id || index} style={styles.hospitalAccordion}>
              ...
              <View style={styles.hospitalHeader}>
                ...
                >
                  <RemoveHospitalCloseIcon width={20} height={20} color="#999" />
                </TouchableOpacity>
              </View>
        ...
        >
          <View style={[
            ...
            {formData.selectedCategory.pharmacy && (
              <AppText style={styles.checkboxTick}><Icon name="check" size={15} color="#ffffff" />
              </AppText>
            )}
          </View>
        ...
        <View style={styles.customerGroupGridContainer}>
          {customerGroups.length > 0 ? (
            customerGroups.map((group) => {
              ...
              const isDisabled = group.customerGroupCode !== 'GOVT';
