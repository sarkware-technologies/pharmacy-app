...
const GroupHospitalRegistrationForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigation.navigate('RegistrationSuccess', {
        customerCode: `GHSP${Math.random().toString().substr(2, 5)}`,
        codeType: 'Group Hospital',
      });
      
    } catch (error) {
      Alert.alert('Error', 'Failed to register group hospital. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFetchGstFromPan = () => {
    Alert.alert('Fetching GST', 'Fetching GST details from PAN...');
    // Mock GST numbers
    setTimeout(() => {
    ...
    >
      ...
      {showGstDropdown && (
        <View style={styles.dropdown}>
          {['GST123456', 'GST789012', 'GST345678'].map((gst, index) => (
            <TouchableOpacity
              key={index}
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          navigation.navigate('HospitalSelector', {
            selectedHospitals: formData.linkedHospital ? [formData.linkedHospital] : [],
            onSelect: (hospitals) => {
              setFormData(prev => ({ ...prev, linkedHospital: hospitals[0] || null }));
            }
          });
        }}
        activeOpacity={0.7}
        ...
        {formData.linkedHospital ? (
          <View style={styles.selectedItem}>
            ...
            <TouchableOpacity
              onPress={(e) => {
                e.stopPropagation();
                setFormData(prev => ({ ...prev, linkedHospital: null }));
              }}
            >
